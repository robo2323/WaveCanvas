...
function Wave(): JSX.Element {
  ...
  const waves: WaveType[] = useMemo(() => ([
    ...
    { ...targetWaveValues, phase: 0, styles: { lineColor: "#bd90ff", dashArray: [6, 3] } }, // Target/non interactive wave
  ]), [amplitude, frequency, phase])

  return (
    <div className="w-full h-screen bg-white flex flex-col items-center">
      {nonInteractive ? (<WaveCanvas waves={[{ ...targetWaveValues, phase: 0, styles: { lineColor: "#680ce9" } }]} animatePhase={animatePhase} phaseAnimationFactor={phaseAnimationFactor} />) : (
        <>
          <WaveCanvas waves={waves} waveSummationStyles={{ showSummation, overlay: overlaySummation }} animatePhase={animatePhase} phaseAnimationFactor={phaseAnimationFactor} />
      ...
      <div className="flex flex-col w-1/3 space-y-4 mt-3">
        ...
        <input className="ml-2" id="nonInteractive" type="checkbox" checked={nonInteractive} onChange={(e) => setNonInteractive(e.currentTarget.checked)} />
        </label>
        <label htmlFor="animationSpeed" className="text-dark">
          (phase) animation speed:
          <input className="ml-2 border-2 border-gray" type="number" min="0" max="0.5" step="0.01" value={phaseAnimationFactor} onChange={(e) => setPhaseAnimationFactor(+e.currentTarget.value)} />
        </label>
        <div className="flex justify-between">
          <label htmlFor="animatePhase" className="text-dark">
            Animate phase?
        <input className="ml-2" id="animatePhase" type="checkbox" checked={animatePhase} onChange={(e) => setAnimatePhase(e.currentTarget.checked)} />
          </label>
          <label htmlFor="showSummation" className="text-dark">
            Show summation of waves?
        <input className="ml-2" id="animatePhase" type="checkbox" checked={showSummation} onChange={(e) => setShowSummation(e.currentTarget.checked)} />
          </label>
          <label htmlFor="overlaySummation" className="text-dark">
            Overlay wave summation?
        <input className="ml-2" id="overlaySummation" type="checkbox" checked={overlaySummation} onChange={(e) => setOverlaySummation(e.currentTarget.checked)} />
          </label>
        </div>
      </div>
